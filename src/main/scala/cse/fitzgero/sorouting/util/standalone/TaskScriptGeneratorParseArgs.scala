...
object TaskScriptGeneratorParseArgs {
  ...
  def parse(args: Seq[String]): (Conf, Seq[String]) = {
    ...
    } yield s"""sbt -mem 12288 "run-main cse.fitzgero.sorouting.app.SORoutingLocalGraphInlineApplication --config ${conf.config()} --network ${conf.network()} --dest ${conf.dest()}/${conf.name()} --win $win --pop $pop --route $route""""
    (conf, scripts)
  }
