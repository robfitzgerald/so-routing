...
object StandardMCTSService extends GraphService {
  override type VertexId = StandardMCTSAlgorithm.VertexId
  override type EdgeId = StandardMCTSAlgorithm.EdgeId
  override type Graph = StandardMCTSAlgorithm.Graph
  type Path = List[SORoutingPathSegment]
  type AlgorithmResult = StandardMCTSAlgorithm.AlgorithmResult
  ...
  override type ServiceConfig = StandardMCTSAlgorithm.AlgorithmConfig
//    {
...
//  }
  ...
  override def runService(graph: Graph, request: ServiceRequest, config: Option[ServiceConfig] = None): Future[Option[ServiceResult]] = {
    ...
    StandardMCTSAlgorithm.runAlgorithm(graph, algRequest, config) match {
