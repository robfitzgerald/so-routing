...
/**
  ...
  */
class BPRCostFunction (capacity: Double, freeFlowSpeed: Double, val zeroValue: Double = 0D) extends CostFunction {
  ...
  override def costFlow(flow: Double): Double = costTerm1 + costTerm2 * pow((flow + zeroValue) / capacity, 4)
