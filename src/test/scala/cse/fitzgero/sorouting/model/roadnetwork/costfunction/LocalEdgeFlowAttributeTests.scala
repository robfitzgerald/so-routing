...
class LocalEdgeFlowAttributeTests extends SORoutingUnitTestTemplate {
  "new LocalEdgeAttributeTests" when {
    "apply with non-empty arguments" should {
      "construct a valid attribute and cost function" in new TestAssets.ValidArgs {
        val result = new LocalEdgeFlowAttribute(fixedFlow, capacity, freeFlowSpeed, distance) with BPRCostFunction
    ...
    "apply with a None argument" should {
      "construct a valid attribute and a cost function that, when missing some data, computes correctly" in new TestAssets.ValidArgs {
        val result1 = new LocalEdgeFlowAttribute(None, capacity, freeFlowSpeed, distance) with BPRCostFunction
        val result2 = new LocalEdgeFlowAttribute(fixedFlow, None, freeFlowSpeed, distance) with BPRCostFunction
        val result3 = new LocalEdgeFlowAttribute(fixedFlow, capacity, None, distance) with BPRCostFunction
        val result4 = new LocalEdgeFlowAttribute(fixedFlow, capacity, freeFlowSpeed, None) with BPRCostFunction
        val result5 = new LocalEdgeFlowAttribute(None, None, None, None) with BPRCostFunction
